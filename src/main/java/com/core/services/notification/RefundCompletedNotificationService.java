...
public class RefundCompletedNotificationService {
	...
	public void notify(RefundCompletedEvent event) {
		if (event.recipientEmail() != null && !event.recipientEmail().isEmpty()) {
			try {
				String html = templateService.render("email/refund-completed", "data", event);

				zohoMailService.send(event.orgId(),event.recipientEmail(), "Refund Completed", html);
			} catch (Exception ex) {
		...
		if (event.recipientMobileNumber() != null && !event.recipientMobileNumber().isEmpty()) {
			try {
				smsService.sendRefundCompletedSms(event.orgId(),event.recipientMobileNumber(), event.bookingId(),
						event.refundAmount());
			} catch (Exception ex) {
